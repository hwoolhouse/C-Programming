...
% Last Modified by GUIDE v2.5 08-Nov-2017 01:35:24
...
function plotData_Callback(hObject, eventdata, handles)
    ...
    setappdata(0,'rollFFT',rollFFT);
    setappdata(0,'pitchFFT',pitchFFT);
    setappdata(0,'yawFFT',yawFFT);
...
function [rollFFTrate,pitchFFTrate,yawFFTrate] = rateCalculations
...
sampleNumber = getappdata(0,'sampleNumber');
    ...
    while (i<=sampleNumber)
        if(i==1)
            ...
            rollFFTrate(i) = rollFFT(i-1)-rollFFT(i);
            pitchFFTrate(i) = pitchFFT(i-1)-pitchFFT(i);
            yawFFTrate(i) = yawFFT(i-1)-yawFFT(i);
        end
        i=i+1;
    end
    i=1;
...
% --- Executes on button press in plotRate.
function plotRate_Callback(hObject, eventdata, handles)
% hObject    handle to plotRate (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of plotRate
